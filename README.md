...
```
テキスト → OpenJTalk (読み変換) → MiniSpeech → メル → ボコーダ → 音声
```
...
MiniSpeech は Duration 展開による非自己回帰 (Non-AR) 音響モデルで、
...
MiniSpeech と VITS (piper-plus) は Duration の扱いが根本的に異なります:

```
MiniSpeech (Duration 整数展開):
...
MiniSpeech では `round → long` で整数化した duration をそのまま `repeat_interleave` に
...
| | piper (VITS) | piper-plus (VITS2) | mini-jtts |
|---|---|---|---|
...
python src/cli/export_onnx_all.py --fs-ckpt fs/enc_d192/fs_2000.pth \
    --voc-ckpt checkpoints/vocos_jsut/vocos_last.pth --voc-type vocos
