...
1. **FastSpeech2 lightning** がテキストからメルスペクトログラム (声の特徴) を生成
...
- FastSpeech2 lightning に**自己アライメント**を追加 (外部ツール VITS・MFA が不要に)
...
```
テキスト → OpenJTalk (読み変換) → FastSpeech2 lightning → メル → SqueezeWave → 音声
```
...
声の特徴は音響モデル (FastSpeech2 lightning) が決めます。ボコーダは話者に依存しません。
...
3. FastSpeech2 lightning 用のマニフェスト (`fs_data/jsut_manifest.json`) を用意:
...
MIT ([LICENSE](LICENSE))。
Vocos, SqueezeWave (NVIDIA WaveGlow, BSD-3), HiFi-GAN, piper を利用 —
