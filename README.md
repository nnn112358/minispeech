...
デフォルトのボコーダは **Vocos** です。**HiFi-GAN**・**MB-iSTFT** は評価・比較用として
同じ条件で学習・評価できます(デフォルト構成 = **MiniSpeechEncoder + Vocos**)。
...
(下表は学習重みのみ。学習品質 mel_l1 の実測は [PLAN.md](PLAN.md)、d64/d384 を含む
ONNX サイズは [onnx_v2/README.md](onnx_v2/README.md) を参照):
...
初期実験で使った WaveGlow 派生の正規化フローボコーダ。小型・高速 (`c64` は CPU で 400 倍超 RT)
でしたが、学習に4段の手順 (NLL → aux → GAN → BN 再キャリブ) を要し、単段 GAN で済む上記3種に
主役を譲りました。ソース・スクリプト一式は **`_unused/squeezewave/`** に退避済み
(active codebase 外、復元手順は [CLAUDE.md](CLAUDE.md) 参照)。
...
既定の Vocos は CPU マルチスレッドで実時間の 100 倍超で動作します。
encoder / vocoder 別の ONNX 実測 (CPU・GPU・fp16・relRMSE) は
[onnx_v2/README.md](onnx_v2/README.md) を唯一の基準とし、
旧 CPU 計測 (1/全スレッド) と AX650N NPU の参考値は [PLAN.md](PLAN.md) を参照してください。
