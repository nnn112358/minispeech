...
### piper/piper-plusとの比較

| | piper/piper-plus | MiniSpeech + SqueezeWave|
|---|---|---|
| 構成 | エンドツーエンド (テキスト→音声を1モデルで) | 2段階 (音響モデル + ボコーダ) |
| エンコーダ | Self-Attention (Transformer) | depthwise-separable Conv のみ |
| デコーダ | HiFi-GAN (モデル内蔵) | SqueezeWave |
| 継続長 | VAE + Flow + MAS | Duration Predictor + 自己アライメント |
| パラメータ数 | ~30M (全体) | 3.08M (MiniSpeech) + ボコーダ |
| 外部依存 | なし (エンドツーエンド) | なし (自己完結で学習可能) |
| ボコーダ交換 | 不可 (一体型) | 可能 (SqueezeWave / Vocos / HiFi-GAN) |
