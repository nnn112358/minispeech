...
| | piper (VITS) | piper-plus | MiniSpeech + ボコーダ (本リポ) |
|---|---|---|---|
| 構成 | End2End | End2End | 2段階 (音響モデル + ボコーダ) |
| エンコーダ | Self-Attention (Transformer) | Self-Attention (Transformer) | depthwise-separable Conv のみ |
| デコーダ | HiFi-GAN | MB-iSTFT | ボコーダを自由に選択可能 |
| 継続長 | VAE + Flow + MAS | VAE + Flow + MAS | Duration Predictor + 自己アライメント |
| G2P | espeak-ng | 独自実装 (OpenJTalk ベース) | OpenJTalk (piper-plus の音素マップを使用) |
| 言語 | 1言語/モデル | 8言語マルチスピーカー | 日本語 (単一話者) |
| パラメータ数 | ~15.7M | ~19.6M | 3.08M (MiniSpeech) + ボコーダ |
| ライセンス | MIT (アーカイブ済) | MIT | MIT |
| ボコーダ交換 | 不可 (一体型) | 不可 (一体型) | 可能 (SqueezeWave / Vocos / HiFi-GAN) |
