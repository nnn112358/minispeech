...
```
テキスト → OpenJTalk g2p → [MiniSpeechEncoder] → 80-mel → [SqueezeWave] → 22050Hz 波形
```

- 共通 mel: `PiperMelFeatures` (sr22050 / n_fft1024 / hop256 / win1024, center=False, log-clamp 1e-5)。音響モデルの出力とボコーダの入力が完全一致。
- 音響モデル: MiniSpeechEncoder (非AR, DurationPredictor のみ, pitch/energy なし)。`src/cli/train_minispeech.py`。
...
MiniSpeechEncoder の継続長取得に VITS MAS が必要だった依存を完全除去。
...
- [ ] 音響モデル高速化: ボコーダ縮小で MiniSpeechEncoder (11ms) が律速に
