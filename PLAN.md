...
```
テキスト → OpenJTalk g2p → [MiniSpeech] → 80-mel → [SqueezeWave] → 22050Hz 波形
```
...
MiniSpeech の継続長取得に VITS MAS が必要だった依存を完全除去。
...
- [x] MiniSpeech 自己アライメント (`--learn-alignment`, VITS 依存除去)
...
- [ ] 音響モデル高速化: ボコーダ縮小で MiniSpeech (11ms) が律速に
